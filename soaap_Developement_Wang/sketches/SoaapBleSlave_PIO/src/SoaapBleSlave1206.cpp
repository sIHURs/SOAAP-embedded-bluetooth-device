...
char *infoThis =
{
    "SOAAP BLE Slave Version 22.11.27-1\r\n"
...
void setup()
{
  ...
  bleCom.setPower(0x000);     // Standard-Sendeleistung für BLE
  ...
  // <10000>                BLE-Master-Poll Time-Out (Mikrosekunden)
...
// ----------------------------------------------------------------------------
// Wenn der Master nicht pollt, dann wiederholt sich der Aufruf mit der
// BLE-Master-Poll-Time-Out (siehe blePoll.begin(...)).
//
RawDataAG   rawDataAG;
RawDataM    rawDataM;
...
bool getValues(PlpType plpType, byte *dest)
{
  ...
  switch(plpType)
  {
    case plptMeas6:
      ...
      locNewVal = true;       // Für lokale Zugriffe (Synchronisierung)
      retv = true;
      break;
    ...
    // ----------------------------------------------------------------------------------
      ...
      transData.tmpFloat = rollVal = proc.getRollValue();
      for(si = 0; si < 4; si++)
        dest[di++] = transData.tmpByteArr[si];

      transData.tmpFloat = pitchVal = proc.getPitchValue();
      for(si = 0; si < 4; si++)
        dest[di++] = transData.tmpByteArr[si];

      transData.tmpFloat = yawVal =  proc.getYawValue();
      for(si = 0; si < 4; si++)
        dest[di++] = transData.tmpByteArr[si];
...
char *chkSensHelp =
{
    ...
    "a Winkelanzeige\r\n"
    "m Winkel zum Master (beim Polling)\r\n"
    "n Winkel zum Master (Hex)\r\n"
#ifdef ProcMeasDebug
    "p Debug <ProcMeas>\r\n"
#endif
};
...
void smCheckSens()
{
  ...
  else if(mon.lastKeyIn == 'H' || mon.lastKeyIn == 'h')
  {
    ...
    sm.enter(smSensToMasterValues);
  else if(mon.lastKeyIn == 'n' || mon.lastKeyIn == 'N')
    sm.enter(smSensToMasterValuesH);
#ifdef ProcMeasDebug
  else if(mon.lastKeyIn == 'p' || mon.lastKeyIn == 'P')
...
void smSensToMasterValues()
{
  ...
  mon.print((char *) "Angles Roll = ");
  sprintf(outValue,"%f  Pitch = ",rollVal);
  mon.print(outValue);
  sprintf(outValue,"%f ",pitchVal);
  mon.println(outValue);
  locNewVal = false;
  sm.enter(smCheckSens);
}

void smSensToMasterValuesH()
{
  if(sm.firstEnter())
  {
    sens.syncValuesAG();
  ...
  mon.print((char *) "Angles Roll = ");
  mon.print((byte *) &rollVal,4,' ');
  mon.print((char *) "  Pitch = ");
  mon.println((byte *) &pitchVal,4,' ');
  locNewVal = false;
  sm.enter(smCheckSens);
}
