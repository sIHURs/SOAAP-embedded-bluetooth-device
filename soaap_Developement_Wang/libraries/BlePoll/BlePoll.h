...
typedef struct _Slave
{
  ...
  dword     cntNakEP;       // Zähler für NAK-Antworten beim Empty-Polling, Negative Acknowledgment 
  dword     cntAckDP;       // Zähler für ACK-Antworten beim Data-Polling, ACKnowledge Character 
  dword     cntLostPdu;     // Zähler für verlorene Telegramme, (pdu: Protocol Data Unit)
...
public:
  ...
  int           getCtrlM(int slIdx, byte *dest);   // von lennard
