...
void BlePoll::begin(ComType typeIn, int adrIn, AppType appType, dword watchDog)
{
  ...
  // --------------------------------------------------------------------------
  //
  PlpCtrl25Ptr ctrlPtr;   // Telegramm Type fuer SOAAP2

  wdTimeOut = watchDog;   // WatchDog-Time-Out in Mikrosekunden

  if(typeIn == ctMASTER)  // Master oder Slave
    master  = true;
  else
  ...
  {
    pduOut.adr5 = 0x53;  //slave 3-byte-Adr 
    pduOut.adr4 = 0x4F;
    pduOut.adr3 = 0x41;

    pduIn.adr5 = 0x53; //??? wieder mal
    pduIn.adr4 = 0x4F;
    pduIn.adr3 = 0x41;
  }
  ...
  else if(appType == atSOAAP2)
  {
    if(master)
    {
      ...
      for(int i = 1; i <= MAXSLAVE; i++)
      {
        ctrlPtr = (PlpCtrl25Ptr) &slaveList[i].control;   //control: struct _PlPduBase  
        ctrlPtr->counter = 0;
...
void BlePoll::smWaitNak()
{
  ...
  if(radio->fin(txmPoll, &crcError))
  {
    ...
    // und es werden die ersten 8 Byte (Header, Len und Adresse[0-5]) geholt
    //
    cntAllRecs++;
...
void BlePoll::smEndEP()
{
  ...
  if(pollMaxNr == 0)
  {
    ...
    else if(epCycleTotal == 0) // @yifan gar keine Slave?
    {
...
void BlePoll::smEndComS() // nicht benutzt
{
  if(pollStop || pollStopped)
  {
    pollStopped = true;
...
// Feststellen, ob Übertragung der Steuerungsdaten erfolgt ist
//
...
int BlePoll::getMeas(int slAdr, byte *dest)
{
  ...
  appId = (PlpType) slavePtr->result.plData[0]; // result ist ein PlPduBase von BlePoll.h (counter/type/plData[29]) @yifan
