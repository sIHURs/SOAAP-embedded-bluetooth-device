...
#include "ComRingBuf.h"
  ...
  bool  ComRingBuf::getByteSnd(byte *dest)
  {
    ...
    *dest = sndBuffer[sbReadIdx];   //dest point to the first element in the array sndBuffer
    sbReadIdx++;
    if(sbReadIdx >= sbSize)
      sbReadIdx = 0;
    return(true);
...
int   ComRingBuf::getCount(int len, byte *buffer)  //return length from the buffer
{
  int  count, i;
  int  tmpInt;
